...
| GAS2-R-022 | Security/dependencies | `OPEN` | Remediación C09-SEC: `pnpm audit --prod --json` baja de 2 critical, 9 high y 3 moderate a 0 critical, 1 high y 0 moderate mediante Next 16.3.3 y Prisma 6.19.3, sin overrides | Los advisories critical de Next y doce hallazgos adicionales quedan fuera del grafo; permanece `deepmerge-ts` 7.1.5 / GHSA-ggr8-5vv4-36mx en el tooling de configuración Prisma | El resultado habilita revisión humana de la demo sintética, pero no acredita seguridad total, production readiness ni despliegue público | Mantener el advisory high abierto y no aceptado; actualizar mediante una versión soportada de Prisma cuando elimine la dependencia o declare compatibilidad con `deepmerge-ts` 8, con validación completa |
