...
| GAS2-R-022 | Security/dependencies | `OPEN` | C09 `pnpm audit --prod --json`: 2 critical, 9 high and 3 moderate advisories in the unchanged dependency graph | Published advisories include Next.js issues relevant to Windows-hosted or image-optimization surfaces plus inherited transitive findings | Blocks any claim that the current build is safe for public deployment; local loopback demo remains synthetic and is not production qualification | Assess and upgrade dependencies in a separate scoped PR, then rerun full validation; C09 adds no dependency and accepts no residual risk |
