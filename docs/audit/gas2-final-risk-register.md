...
| GAS2-R-015 | Integration | `DEFERRED` | No productive external/FHIR adapter or contract; ADR-0018 is documentary/read-only, while the bounded CALL-E REST adapter is server-only, disabled and has no live entrypoint | No real interoperability or live external integration | Does not block local demo; blocks integration claims | DEC-018 plus approved contract/profile first; CALL-E remains governed separately by DEC-019 |
...
| GAS2-R-021 | External sandbox / privacy / safety | `BLOCKED_BY_DECISION` | [ADR-0019](../adr/0019-calle-hackathon-sandbox-boundary.md), C00 estático divergente, adapter REST C02 `IMPLEMENTED_DISABLED`, core C03, Relays C04/C05 y gobernanza de resultados C06 locales/sintéticos, HAZ-GAS-021–038 y GAP-DCB-025 | Controles C02–C06 `IMPLEMENTED_UNVALIDATED`; C06 reduce ambigüedad local con mapeo exacto, schema estricto, misma referencia de reconciliación, persistencia minimizada y revisión obligatoria, pero Calls 0.6.0 no expone `resultValidation`; siguen ausentes SDK, resolver/policy/attestation productivos, entrypoint y llamadas; persisten cancelación, intentos físicos, autoridad, voz y términos | C04–C06 solo locales/sintéticos con fixtures/fakes; `CALL_E_SANDBOX = PASS` se limita a esa definición; no telefonía, autoasignación o resolución; sandbox REST disabled; `LIVE_PROOF = NOT_EVIDENCED`; demo pública LIVE OFF; datos reales/piloto/producción NO_GO | DEC-019 y dependencias: fuente canónica real de destino/attestation, licencia SDK separada, términos/garantías REST, gates y pruebas live futuras; due diligence de DPA/processor, transferencias/localización, retención, grabaciones/transcripts, seguridad/SLA/continuidad, incidentes, ePrivacy/telecom, procurement, base jurídica, consentimiento y supplier exit; sin aceptación residual |
| GAS2-R-022 | Security/dependencies | `OPEN` | C09 histórico registró 2 critical, 9 high y 3 moderate en su grafo sin cambios; la remediación C09-SEC ejecutó `pnpm audit --prod --json` y obtuvo 0 critical, 1 high y 0 moderate mediante Next 16.3.3, Prisma 6.19.3 y resolución `browserslist` 4.28.9, sin overrides | Los dos advisories critical de Next y once hallazgos adicionales quedan fuera del grafo actual; permanece `deepmerge-ts` 7.1.5 / GHSA-ggr8-5vv4-36mx en el tooling de configuración Prisma | El resultado habilita revisión humana de la demo sintética, pero no acredita seguridad total, production readiness ni despliegue público | Mantener el advisory high abierto y no aceptado; actualizar mediante una versión soportada de Prisma cuando elimine la dependencia o declare compatibilidad con `deepmerge-ts` 8, con validación completa |
