...
| GAP-DCB-025 | Dependencia externa futura del sandbox CALL-E | Licencia SDK, términos/garantías REST, intentos físicos, fuente canónica real de destino/attestation, policy temporal productiva, contención de voz, retención y gates live sin resolver ni probar | [ADR-0019](../../adr/0019-calle-hackathon-sandbox-boundary.md), síntesis C00 y evidencia C02/C03/C04/C05; adapter server-only `IMPLEMENTED_DISABLED`, autoridad tipada, preview sin red, HMAC, confirmación CAS one-use, lifecycle/audit mínimos y dos Relays locales sintéticos separados con resultados cerrados/revisión humana `IMPLEMENTED_UNVALIDATED`; resolver no sintético deny-all, sin SDK, entrypoint live ni llamadas | Mantener SDK y live prohibidos; resolver DEC-019 y autoridad institucional antes de cualquier ensayo live/real; incorporar fuente canónica única sin persistir teléfono en Relay; validar proveedor, privacidad y safety sin datos reales | Responsable del proyecto y revisores técnicos/jurídicos por designar para sandbox; CSO y autoridades clínicas/institucionales futuros para cualquier uso real | `OPEN`; C04/C05 reducen gaps técnicos sintéticos pero no demuestran destinatario real, controles live, validación clínica, transferencia o aceptación residual |
