...
| GAP-DCB-025 | Dependencia externa futura del sandbox CALL-E | Licencia SDK, términos/garantías REST, intentos físicos, fuente canónica real de destino/attestation, policy temporal productiva, contención de voz, retención y gates live sin resolver ni probar | [ADR-0019](../../adr/0019-calle-hackathon-sandbox-boundary.md), síntesis C00 y evidencia C02–C06; adapter server-only `IMPLEMENTED_DISABLED`, autoridad tipada, preview sin red, HMAC, confirmación CAS one-use, lifecycle/audit mínimos, dos Relays locales separados y gobernanza local de errores/resultados con reconciliación y review `IMPLEMENTED_UNVALIDATED`; resolver no sintético deny-all, sin SDK, entrypoint live ni llamadas | Mantener SDK y live prohibidos; resolver DEC-019 y autoridad institucional antes de cualquier ensayo live/real; incorporar fuente canónica única sin persistir teléfono en Relay; validar proveedor, privacidad y safety sin datos reales | Responsable del proyecto y revisores técnicos/jurídicos por designar para sandbox; CSO y autoridades clínicas/institucionales futuros para cualquier uso real | `OPEN`; C06 reduce ambigüedad técnica sintética pero Calls 0.6.0 no expone `resultValidation` y no demuestra destinatario, intentos, controles live, validación clínica, transferencia o aceptación residual |
