...
| GAP-DCB-025 | Dependencia externa futura del sandbox CALL-E | Licencia SDK, términos/garantías REST, intentos físicos, destino/autoridad, contención de voz, retención y gates live sin resolver ni probar | [ADR-0019](../../adr/0019-calle-hackathon-sandbox-boundary.md), síntesis C00 y evidencia C02 REST; adapter server-only `IMPLEMENTED_DISABLED`, HMAC, claim atómico, mapper mínimo y pruebas fake `IMPLEMENTED_UNVALIDATED`; sin SDK, entrypoint ni llamadas | Mantener SDK prohibido; resolver DEC-019 y aclaraciones aplicables antes de cualquier ensayo live/publicación; implementar autoridad/confirmación solo en fase separada; validar proveedor, privacidad y safety sin usar datos reales | Responsable del proyecto y revisores técnicos/jurídicos por designar para sandbox; CSO y autoridades clínicas/institucionales futuros para cualquier uso real | `OPEN`; controles técnicos no validados live, ninguna validación clínica, transferencia o aceptación residual |
