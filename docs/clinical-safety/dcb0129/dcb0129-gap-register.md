...
| GAP-DCB-025 | Dependencia externa futura del sandbox CALL-E | Licencia SDK, términos/garantías REST, intentos físicos, fuente canónica real de destino/attestation, policy temporal productiva, contención de voz y gates live sin resolver ni probar; due diligence de DPA/processor, transferencias/localización, retención, grabaciones/transcripts, seguridad/SLA/continuidad, incidentes, ePrivacy/telecom, procurement, base jurídica, consentimiento y supplier exit pendiente | [ADR-0019](../../adr/0019-calle-hackathon-sandbox-boundary.md), síntesis C00 y evidencia C02–C06; adapter server-only `IMPLEMENTED_DISABLED`, autoridad tipada, preview sin red, HMAC, confirmación CAS one-use, lifecycle/audit mínimos, dos Relays locales separados y gobernanza local de errores/resultados con reconciliación y review `IMPLEMENTED_UNVALIDATED`; resolver no sintético deny-all, sin SDK, entrypoint live ni llamadas. GAS does not persist CALL-E transcripts. | Mantener SDK y live prohibidos; resolver DEC-019 y autoridad institucional antes de cualquier ensayo live/real; incorporar fuente canónica única sin persistir teléfono en Relay; completar due diligence y validar proveedor, privacidad y safety sin datos reales, sin afirmar comportamiento de retención del proveedor | Responsable del proyecto y revisores técnicos/jurídicos por designar para sandbox; CSO y autoridades clínicas/institucionales futuros para cualquier uso real | `OPEN`; `CALL_E_SANDBOX = PASS` solo para la evidencia local/sintética y no reduce el gap: Calls 0.6.0 no expone `resultValidation`, `LIVE_PROOF = NOT_EVIDENCED` y no se demuestran destinatario, intentos, controles live, validación clínica, transferencia o aceptación residual |

La ampliación C01 del 27-08-2026 incorpora 18 escenarios futuros CALL-E al
Hazard Log canónico (38 entradas en total). No cierra GAP-DCB-007/010 ni reduce
GAP-DCB-023/024: el núcleo de compromisos y la validación clínica/human factors
conservan sus límites. El registro no afirma conformidad DCB0129/DCB0160.
