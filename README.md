...
La gobernanza del episodio es una proyección no persistida sobre `DischargeEpisode`, responsables activos, protocolo exacto, evidencia técnica de identidad, avisos no terminales y tareas abiertas. La API y el resumen profesional muestran blockers mediante códigos, IDs, estados y versiones, sin copiar explicaciones, respuestas ni resúmenes. DEC-002 permanece pendiente: el cierre está `NOT_AUTHORIZED`, y política ausente, error o estado inconsistente también fallan de forma cerrada. Avisos y tareas son obligaciones organizativas visibles, no criterios clínicos definitivos.

El motor de avisos usa un DSL JSON v1 validado con inputs permitidos explícitos, ventanas temporales y operadores deterministas. Cada evaluación conserva versión, timestamp, snapshot estructurado, hash reproducible y resultado `matched`, `not-matched` o `abstained`; si falta un input requerido se abstiene. `Idempotency-Key` evita duplicar una evaluación o aviso por reintento y rechaza reutilizar la clave con otro payload. Un boundary canónico de procedencia v1 distingue evidencia fuente de derivados y reconstruye `fuente → RuleEvaluation → Alert` mediante IDs, tiempos, productores y versiones existentes, sin copiar respuestas, observaciones, planes, explicaciones ni notas. `Alert.inputReferences` conserva su raíz array histórica: los avisos nuevos guardan dentro un lineage v1 y los lectores marcan los arrays antiguos como no versionados o los formatos desconocidos como inválidos. Un aviso coincidente permanece `open` hasta una revisión humana; las revisiones son append-only y no crean tareas ni acciones clínicas. Para crear explícitamente una tarea derivada, una policy pura y fail-closed combina la revisión histórica con el rol activo y la responsabilidad actual del acting actor dentro del mismo unit of work. La decisión no se persiste ni ejecuta la acción; `AlertReview`, `Task`/`TaskEvent` y `AuditEvent` conservan sus historias separadas. El seed incluye cuatro reglas `draft` sintéticas y no aprobadas; DEC-008 mantiene pendiente su validación clínica local.

La cola profesional filtra episodios visibles por estado, fecha, responsable y elementos pendientes. Muestra solo el último check-in sin respuestas, los avisos no cerrados con versión/origen y las tareas del episodio. Una tarea siempre pertenece a un episodio y puede referenciar un aviso del mismo episodio; solo se crea mediante una petición profesional explícita. Si no tiene aviso es iniciación humana directa; si lo tiene, primero exige RBAC y responsabilidad actuales mediante un guard independiente y después `CREATE_TASK_FROM_REVIEWED_ALERT` autorizado con evidencia real de `AlertReview`. Reviewer y creador pueden ser personas distintas. La evidencia no copia payload clínico ni texto libre: contiene identificadores técnicos minimizados, timestamps y referencias de versión, con identidades exclusivamente sintéticas en el demo. El estado administrativo `actioned` no acredita por sí solo una tarea: esa evidencia reside en `Task`/`TaskEvent`.

`TECHNICAL TASK ACCOUNTABILITY = implemented`: se proyecta sin tabla nueva; `Task` conserva estado actual y `TaskEvent` la historia append-only. Creador, assignee actual, actor de evento, resolver y responsables del episodio se muestran como conceptos distintos. La cadena valida revisiones, estados, timestamp de creación y extremos de assignment/reassignment; una incoherencia falla explícitamente. Un assignee revocado después de una asignación válida permanece en la historia y se marca como no autorizado actualmente, sin reasignación automática. El invariant de la workqueue es `episodio → User de participantes únicos ordenados globalmente por ID → RoleAssignment de actor/target → tarea/evento`; así create-assigned, assign y reassign se serializan frente a revocación y cruces actor/target tanto dentro de un episodio como entre episodios. Pares de episodios e identidades disjuntos continúan en paralelo. Assignment no implica acceptance ni autoridad exclusiva para actuar. `expectedRevision`, idempotencia e índices únicos evitan dobles procesamientos silenciosos.

`INSTITUTIONAL RESPONSIBILITY / ACCOUNTABILITY POLICY = not validated, conditioned on DEC-017`: el software no sabe quién debería actuar y no implementa acceptance, equipo/turno, suplencia, autoassignment, prioridad, SLA o escalado. Tampoco hay comunicaciones reales, derivación, recomendación, SBAR automático o cierre automático.

El workspace del episodio incluye una vista `Evidencia / Trazabilidad` estrictamente read-only. Compone `DischargeEpisode`/`EpisodeTransition`, `EpisodeGovernanceView`, procedencia V1, `AlertReview`, `TaskAccountabilityProjection` y referencias de `AuditEvent` sin crear otra fuente de verdad ni copiar payload clínico. Evidencia y gobernanza se leen y evalúan dentro de una única transacción PostgreSQL `REPEATABLE READ`, por lo que tareas o roles concurrentes no producen snapshots mezclados. La vista valida el lineage canónico persistido y deja claro que sus referencias fuente fueron verificadas durante `RuleEvaluation`; no vuelve a consultar la fila fuente durante la lectura de evidencia. `RuleEvaluation.inputHash` se usa internamente para contrastar el lineage, pero no se publica. Las colecciones están limitadas explícitamente a 100 referencias y cualquier truncamiento se declara como evidencia parcial, salvo contradicciones ya visibles, que permanecen inconsistentes. `COMPLETE`, `PARTIAL`, `INCONSISTENT`, `NOT_APPLICABLE` y `UNAVAILABLE` describen solo integridad/disponibilidad técnica; no certifican seguridad clínica, cumplimiento o conformidad regulatoria. La decisión histórica por instancia de `HumanAuthorizationPolicy` y el rol histórico del reviewer no están persistidos y se declaran indisponibles.
